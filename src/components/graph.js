...
function getClosestPointOnSegment(px, py, x1, y1, x2, y2) {
    if (typeof px !== 'number' || typeof py !== 'number' || typeof x1 !== 'number' || typeof y1 !== 'number' || typeof x2 !== 'number' || typeof y2 !== 'number' ||
        ...
        return {x: NaN, y: NaN, distSq: Infinity};
    }
    const dx = x2 - x1, dy = y2 - y1, lenSq = dx * dx + dy * dy;
    if (lenSq < 1e-9) {
        const dSq = distance(px, py, x1, y1) ** 2;
        return {x: x1, y: y1, distSq: isNaN(dSq) ? Infinity : dSq};
    }
...
function parseSvgPathRoad(roadData, roadId, floorIndex) {
    ...
    commands.forEach((cmd) => {
        ...
        const isCurrentValid = !isNaN(currentX) && !isNaN(currentY);

        switch (type) {
            case 'M': {
                if (args.length >= 2 && !isNaN(args[0]) && !isNaN(args[1])) {
                    if (cmd[0] === 'm' && isCurrentValid) {
                        nextX = currentX + args[0];
                        nextY = currentY + args[1];
                    } else {
                        nextX = args[0];
                        nextY = args[1];
                    }
                    if (!isNaN(nextX) && !isNaN(nextY)) {
                        ...
                        for (let i = 2; i < args.length; i += 2) {
                            if (args.length > i + 1 && !isNaN(args[i]) && !isNaN(args[i + 1])) {
                                let pointX, pointY;
                                if (cmd[0] === 'm' && !isNaN(currentX)) {
                                    pointX = currentX + args[i];
                                    pointY = currentY + args[i + 1];
                                } else {
                                    pointX = args[i];
                                    pointY = args[i + 1];
                                }
                                if (!isNaN(pointX) && !isNaN(pointY) && !isNaN(currentX)) {
            ...
            case 'H': {
                ...
                for (const arg of args) {
                    if (!isNaN(arg)) {
                        ...
                        if (!isNaN(pointX)) {
                            points.push({x: pointX, y: currentY});
                            currentX = pointX;
                        }
                    }
                }
            ...
            case 'V': {
                ...
                for (const arg of args) {
                    if (!isNaN(arg)) {
                        ...
                        if (!isNaN(pointY)) {
                            points.push({x: currentX, y: pointY});
                            currentY = pointY;
                        }
                    }
                }
            ...
            case 'Z': {
                if (isCurrentValid && !isNaN(startX) && !isNaN(startY)) {
            ...
            case 'A': {
                ...
                if (lastCoordIndex >= 0 && !isNaN(args[lastCoordIndex]) && !isNaN(args[lastCoordIndex + 1])) {
                    let tempX = NaN, tempY = NaN;
                    if (cmd[0] === cmd[0].toLowerCase() && isCurrentValid) {
                        tempX = currentX + args[lastCoordIndex];
                        tempY = currentY + args[lastCoordIndex + 1];
                    } else {
                        tempX = args[lastCoordIndex];
                        tempY = args[lastCoordIndex + 1];
                    }
    ...
    const uniquePoints = points.filter((p, i, arr) => {
        ...
        if (i === 0) return true;
        const prev = arr[i - 1];
        if (isNaN(prev.x) || isNaN(prev.y)) return true;
        ...
        for (let i = 0; i < uniquePoints.length - 1; i++) {
            ...
            if (!isNaN(p1.x) && !isNaN(p1.y) && !isNaN(p2.x) && !isNaN(p2.y)) {
                if (Math.abs(p1.x - p2.x) > 1e-6 || Math.abs(p1.y - p2.y) > 1e-6) {
                    uniqueSegments.push({x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, roadId, floorIndex});
                }
            }
        }
    }
    ...
    layers.forEach((layer, floorIndex) => { /* ... (без изменений) ... */
        ...
        (layer.roads || []).filter(road => road && !isNaN(road.x1) && !isNaN(road.y1) && !isNaN(road.x2) && !isNaN(road.y2))
            .forEach((road, roadIndex) => {
                ...
                nodeToRoadIdMap.set(startNodeId, roadId);
                nodeToRoadIdMap.set(endNodeId, roadId);
                ...
                try {
                    ...
                    if (points.length > 1 && segments.length > 0) {
                        ...
                        points.forEach((p, i) => {
                            ...
                            nodeToRoadIdMap.set(nodeId, roadId);
                        ...
                        if (startPoint) roadEndpoints.get(floorIndex).push({
                            id: startNodeId,
                            x: startPoint.x,
                            y: startPoint.y,
                            roadId
                        });
                        if (endPoint && startNodeId !== endNodeId) roadEndpoints.get(floorIndex).push({
                            id: endNodeId,
                            x: endPoint.x,
                            y: endPoint.y,
                            roadId
                    ...
                    console.error(`Ошибка парсинга SVG дороги ${roadId}:`, e);
                }
            });
        (layer.vectors || []).filter(v => v && v.type === 'icon' && v.id).forEach(icon => {
            ...
            if (coords && !isNaN(coords.x) && !isNaN(coords.y)) {
                ...
                if (!graph.has(nodeId)) graph.set(nodeId, new Map());
                nodeToRoadIdMap.set(nodeId, `icon_${floorIndex}`);
                ...
                if (stairMatch) {
                    ...
                    stairIconsMap.get(logicalId).push({
                        id: nodeId,
                        x: coords.x,
                        y: coords.y,
                        floorIndex: currentFloorActualIndex
                    });
                }
            }
        });
        ...
        if (nodeId.startsWith('icon-')) {
                ...
                if ((roadNodeId.startsWith('road-') || roadNodeId.startsWith('path-')) && roadNodeInfo.floorIndex === currentFloor) {
                    ...
                    if (!isNaN(distSq) && distSq < closestDistSq) {
                        ...
                        connectionCandidate = {
                            type: 'node',
                            id: roadNodeId,
                            point: {x: roadNodeInfo.x, y: roadNodeInfo.y},
                            distSq: distSq
                        };
                    }
                }
            ...
            allRoadSegments.filter(seg => seg.floorIndex === currentFloor).forEach((segment) => {
                ...
                if (closestOnSeg.distSq < closestDistSq) {
                    ...
                    connectionCandidate = {
                        type: 'segment',
                        id: {startNodeId: segment.startNodeId, endNodeId: segment.endNodeId},
                        point: {x: closestOnSeg.x, y: closestOnSeg.y},
                        distSq: closestOnSeg.distSq,
                        segmentData: segment
                    };
                }
            });
            if (connectionCandidate) {
                ...
                if (connectionCandidate.type === 'node') {
                    ...
                    const {
                        ...
                        y2: sy2,
                        roadId: _segRoadId
                    } = connectionCandidate.segmentData;
                    ...
                    if (virtualNodeMap.has(vNodeKey)) {
                        ...
                        if (!isNaN(weightStartToVirtual) && !isNaN(weightVirtualToEnd) && weightStartToVirtual >= 0 && weightVirtualToEnd >= 0 && (segWeight < 1e-6 || Math.abs(weightStartToVirtual + weightVirtualToEnd - segWeight) < 1e-3)) {
                            removeEdge(segStartId, segEndId);
    ...
    layers.forEach((_, floorIndex) => { /* ... (без изменений, использует PROXIMITY_CONNECTION_THRESHOLD_SQ) ... */
        ...
        for (let i = 0; i < floorNodes.length; i++) {
            for (let j = i + 1; j < floorNodes.length; j++) {
                ...
                if (!roadId1 || !roadId2 || roadId1 === roadId2 || (id1.startsWith('icon-') && id2.startsWith('icon-'))) continue;
                if (!node1Info || !node2Info || isNaN(node1Info.x) || isNaN(node1Info.y) || isNaN(node2Info.x) || isNaN(node2Info.y)) continue;
