...
function RoomInfoModal({room, onClose}) {
    ...
    // Можно выбрать как точку маршрута, если это не тех. помещение И (есть имя ИЛИ есть описание)
    const canBeRoutePoint = room &&
    ...
    const getDisplayContent = () => {
        ...
        if (room.type === "icon") {
            const displayValue = room.name || room.description || `ID: ${room.id}`;
            return (
                <div className="info-grid">
                    <span className="info-label">Место</span>
                    <span className="info-value">{displayValue}</span>
                    {/* Показываем description только если он отличается от основного отображаемого текста */}
                    {room.description && room.description !== displayValue && (
                        <>
                            <span className="info-label">Доп.</span>
        ...
        // Комнаты: Приоритет у name, потом description
        const mainLabel = room.name ? "Помещение" : (room.description ? "Описание" : "Объект");
        const mainValue = room.name || room.description;
        const showAdditionalInfo = room.name && room.description && room.name !== room.description;

        return (
            <div className="info-grid">
                {mainValue && (
                    <>
                        <span className="info-label">{mainLabel}</span>
                        <span className="info-value">{mainValue}</span>
                    </>
                )}
                {showAdditionalInfo && (
                    <>
